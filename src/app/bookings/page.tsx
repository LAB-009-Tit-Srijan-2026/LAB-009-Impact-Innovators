...
import { Hotel, Plane, Activity, Search, QrCode, Download, X, Star, Calendar, Users } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="text-xs text-purple-600 font-medium mb-5">🇮🇳 India Travel Ticket</p>
        <div className="w-48 h-48 mx-auto bg-purple-50 rounded-2xl flex items-center justify-center mb-5 border-2 border-dashed border-purple-200">
          <div className="grid grid-cols-5 gap-1 p-3">
            {[...Array(25)].map((_, i) => (
              <div key={i} className={`w-6 h-6 rounded-sm ${Math.random() > 0.5 ? 'bg-purple-800' : 'bg-white'}`} />
            ))}
          </div>
        </div>
        <p className="text-xs text-gray-400 mb-4">Check-in ke waqt ye QR dikhao</p>
        <button className="w-full text-white font-semibold py-2.5 rounded-xl text-sm"
          style={{ background: 'linear-gradient(135deg, #7c3aed, #db2777)' }}>
          Ticket Download Karo
        </button>
      </motion.div>
    </motion.div>
  );
        ...
        <div className="flex-1 p-5">
          ...
          <div className="flex flex-wrap gap-4 mb-4">
            <div className="flex items-center gap-1.5 text-sm text-gray-600">
              ...
              {booking.checkIn !== booking.checkOut && ` → ${formatDate(booking.checkOut)}`}
            </div>
            <div className="flex items-center gap-1.5 text-sm text-gray-600">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button className="flex items-center gap-1.5 px-3 py-2 bg-gray-100 text-gray-600 rounded-xl text-xs font-semibold hover:bg-gray-200 transition-colors">
              <Download className="w-3.5 h-3.5" /> Invoice
            </button>
            {booking.status === 'confirmed' && (
              <button className="flex items-center gap-1.5 px-3 py-2 bg-red-50 text-red-600 rounded-xl text-xs font-semibold hover:bg-red-100 transition-colors ml-auto">
                Cancel Karo
              </button>
            )}
          </div>
